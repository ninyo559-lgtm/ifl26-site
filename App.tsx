...
import { LeagueType, Player, Fixture, PlayerStats, SeasonHistoryEntry, HallOfFameEntry, PlayoffMatch, PlayoffBrackets, CLGroup } from './types';
import { generateLeagueCommentary } from './services/geminiService';

const STORAGE_KEYS = {
  PLAYERS: 'ifl_stable_players_list',
  PREMIER_IDS: 'ifl_stable_premier_player_ids',
  NATIONAL_IDS: 'ifl_stable_national_player_ids',
  CL_GROUPS: 'ifl_stable_champions_groups',
  FIXTURES: 'ifl_stable_league_fixtures',
  HOF: 'ifl_stable_hall_of_fame',
  MONTH: 'ifl_stable_current_season_month',
  PLAYOFFS: 'ifl_stable_playoff_brackets',
  HISTORY: 'ifl_stable_seasons_history',
};
...
const App: React.FC = () => {
  ...
  const calculateTable = (league: LeagueType, groupId?: string): PlayerStats[] => {
    ...
    const safePremierIds = premierPlayerIds || [];
    const safeNationalIds = nationalPlayerIds || [];
    const safeClGroups = clGroups || [];
    const safeFixtures = fixtures || [];
    const safePlayers = players || [];

    if (league === 'premier') leaguePlayerIds = safePremierIds;
    else if (league === 'national') leaguePlayerIds = safeNationalIds;
    else if (league === 'champions' && groupId) {
      const group = safeClGroups.find(g => g.id === groupId);
  ...
  const archiveSeason = (newSeasonName: string) => {
    ...
    const newHistoryEntry: SeasonHistoryEntry = {
      id: Date.now().toString(), seasonName: sName, premierTable: premierStats, nationalTable: nationalStats,
      premierWinner: findWinner(safePlayoffs.premier), nationalWinner: findWinner(safePlayoffs.national), championsWinner: findWinner(safePlayoffs.champions),
      timestamp: new Date().toISOString()
    };
    setHistory(prev => [...(prev || []), newHistoryEntry]);
    setFixtures([]); setPlayoffs({ premier: [], national: [], champions: [] }); setPremierPlayerIds([]); setNationalPlayerIds([]);
    setClGroups([{ id: '1', name: 'בית 1', playerIds: [] }, { id: '2', name: 'בית 2', playerIds: [] }, { id: '3', name: 'בית 3', playerIds: [] }, { id: '4', name: 'בית 4', playerIds: [] }]);
    setCurrentMonth(newSeasonName); setShowArchiveModal(false);
  };
  ...
  const getPlayerCareerStats = (playerId: string) => {
    ...
    stats.titles += safeHOF.filter(entry => entry.playerName === playerName).length;
    return stats;
  };
  ...
  const handleRemoveFromLeague = (id: string, league: LeagueType, groupId?: string) => {
  ...
  const addExistingPlayerToLeague = (playerId: string, league: LeagueType, groupId?: string) => {
    ...
    if (league === 'premier' || league === 'national') {
      ...
      const updated = [...list, playerId]; setter(updated);
  ...
  const startLeaguePlayoffs = (league: LeagueType) => {
    if (league === 'champions') { startChampionsPlayoffs(); return; }
  ...
  const startChampionsPlayoffs = () => {
    const groups = clGroupStats;
    if (groups.some(g => g.stats.length < 2)) return;
    const qf: PlayoffMatch[] = [
      { id: Math.random().toString(36), round: 'quarter', player1Id: groups[0].stats[0].id, player2Id: groups[1].stats[1].id, score1: null, score2: null, winnerId: null, index: 0 },
      { id: Math.random().toString(36), round: 'quarter', player1Id: groups[2].stats[0].id, player2Id: groups[3].stats[1].id, score1: null, score2: null, winnerId: null, index: 1 },
      { id: Math.random().toString(36), round: 'quarter', player1Id: groups[1].stats[0].id, player2Id: groups[0].stats[1].id, score1: null, score2: null, winnerId: null, index: 2 },
      { id: Math.random().toString(36), round: 'quarter', player1Id: groups[3].stats[0].id, player2Id: groups[2].stats[1].id, score1: null, score2: null, winnerId: null, index: 3 },
    ];
    const sf: PlayoffMatch[] = [{ id: Math.random().toString(36), round: 'semi', player1Id: null, player2Id: null, score1: null, score2: null, winnerId: null, index: 0 }, { id: Math.random().toString(36), round: 'semi', player1Id: null, player2Id: null, score1: null, score2: null, winnerId: null, index: 1 }];
    const fn: PlayoffMatch[] = [{ id: Math.random().toString(36), round: 'final', player1Id: null, player2Id: null, score1: null, score2: null, winnerId: null, index: 0 }];
    setPlayoffs(prev => ({ ...(prev || { premier: [], national: [], champions: [] }), champions: [...qf, ...sf, ...fn] }));
  };
  ...
  return (
    <div className="min-h-screen bg-black text-white pb-20 selection:bg-blue-600" dir="rtl">
      <div className="fixed inset-0 pointer-events-none -z-10 overflow-hidden">
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-blue-600/5 blur-[120px]" /><div className="absolute bottom-0 left-0 w-[600px] h-[600px] bg-blue-900/5 blur-[150px]" />
      </div>
        ...
        {activeTab === 'leagues' && (
          <section className="space-y-12">
            <div className="bg-slate-900/40 p-8 rounded-[3rem] border border-white/5 flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl relative overflow-hidden">
               ...
               <button disabled={isGeneratingCommentary} onClick={() => { setIsGeneratingCommentary(true); generateLeagueCommentary(currentMonth, premierStats, safeHistory).then(setCommentary).finally(() => setIsGeneratingCommentary(false)); }} className="bg-blue-600 hover:bg-blue-500 text-white px-8 py-4 rounded-2xl font-black text-xs shadow-xl disabled:opacity-50 transition-all active:scale-95 shrink-0 z-10">{isGeneratingCommentary ? "מנתח..." : "עדכן סיקור AI"}</button>
            </div>
           ...
           <div className="relative w-full max-w-sm animate-in zoom-in-95 duration-300">
              <div className="bg-slate-900 border border-blue-500/40 p-1 rounded-[3.5rem] shadow-2xl overflow-hidden">
                 <div className="bg-slate-950 rounded-[3.2rem] p-12 flex flex-col items-center text-center space-y-6">
                    ...
                    <div className="flex flex-col gap-3 w-full"><button onClick={() => { const val = (document.getElementById('new-season-name') as HTMLInputElement).value; if(val) archiveSeason(val); else alert('נא להזין שם עונה'); }} className="w-full py-5 bg-blue-600 text-white rounded-[2rem] font-black text-xs shadow-xl active:scale-95">סגור עונה ופתח חדשה</button><button onClick={() => setShowArchiveModal(false)} className="w-full py-5 bg-slate-900 text-slate-500 rounded-[2rem] font-black text-xs border border-white/5 active:scale-95">ביטול</button></div>
                 </div>
              </div>
           </div>
        </div>
      ...
      {historyToDelete && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-6 backdrop-blur-2xl">
           <div className="absolute inset-0 bg-black/90" onClick={() => setHistoryToDelete(null)} />
           <div className="relative w-full max-sm animate-in zoom-in-95 duration-300">
              <div className="bg-slate-900 border border-red-500/40 p-1 rounded-[3.5rem] shadow-2xl overflow-hidden">
                 <div className="bg-slate-950 rounded-[3.2rem] p-12 flex flex-col items-center text-center space-y-6">
      ...
      {playerToRemoveFromLeague && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 backdrop-blur-xl">
           <div className="absolute inset-0 bg-black/90" onClick={() => setPlayerToRemoveFromLeague(null)} />
           <div className="relative w-full max-sm animate-in zoom-in-95 duration-300">
              <div className="bg-slate-900 border border-blue-500/40 p-1 rounded-[3.5rem] shadow-2xl overflow-hidden">
                 <div className="bg-slate-950 rounded-[3.2rem] p-12 flex flex-col items-center text-center">
                    <div className="w-20 h-20 rounded-full bg-blue-600/10 flex items-center justify-center mb-6 border border-blue-500/20"><MinusCircle className="text-blue-500" size={40} /></div>
                    <h3 className="text-2xl font-black mb-4 text-white italic text-right w-full">הסרה מליגה בלבד</h3>
                    <p className="text-slate-400 text-sm font-bold mb-8 leading-relaxed text-right w-full">האם להסיר את <span className="text-blue-500 font-black">{playerToRemoveFromLeague.player.name}</span>?</p>
                    <div className="flex flex-col gap-4 w-full"><button onClick={() => handleRemoveFromLeague(playerToRemoveFromLeague.player.id, playerToRemoveFromLeague.league, playerToRemoveFromLeague.groupId)} className="w-full py-5 bg-blue-600 text-white rounded-[2rem] font-black text-xs shadow-xl active:scale-95">כן, הסר</button><button onClick={() => setPlayerToRemoveFromLeague(null)} className="w-full py-5 bg-slate-900 text-slate-400 rounded-[2rem] font-black text-xs border border-white/5 active:scale-95">ביטול</button></div>
                 </div>
              </div>
           </div>
        </div>
      ...
      {playerToDelete && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 backdrop-blur-xl">
           <div className="absolute inset-0 bg-black/90" onClick={() => setPlayerToDelete(null)} />
           <div className="relative w-full max-sm animate-in zoom-in-95 duration-300">
              <div className="bg-slate-900 border border-red-500/40 p-1 rounded-[3.5rem] shadow-2xl overflow-hidden">
                 <div className="bg-slate-950 rounded-[3.2rem] p-12 flex flex-col items-center text-center">
                    <div className="w-24 h-24 rounded-full bg-red-600/10 flex items-center justify-center mb-8 border border-red-500/20"><AlertTriangle className="text-red-500" size={48} /></div>
                    <h3 className="text-3xl font-black mb-4 text-white tracking-tighter italic text-right w-full">מחיקה סופית מהמערכת</h3>
                    <p className="text-slate-400 text-sm font-bold mb-10 leading-relaxed text-right w-full">האם למחוק את <span className="text-red-500 font-black">{playerToDelete.name}</span>?</p>
                    <div className="flex flex-col gap-4 w-full"><button onClick={() => executePlayerDeletion(playerToDelete.id)} className="w-full py-5 bg-red-600 text-white rounded-[2rem] font-black text-xs shadow-xl active:scale-95">כן, מחק</button><button onClick={() => setPlayerToDelete(null)} className="w-full py-5 bg-slate-900 text-slate-400 rounded-[2rem] font-black text-xs border border-white/5 active:scale-95">לא</button></div>
                 </div>
              </div>
           </div>
        </div>
      ...
      {showPinModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 backdrop-blur-md">
           ...
           <div className="bg-slate-900 p-12 rounded-[4rem] border border-white/10 shadow-2xl relative w-full max-w-sm text-center animate-in zoom-in-95 duration-300">
      ...
      <footer className="mt-20 border-t border-white/5 py-16 opacity-20 text-center"><div className="flex flex-col items-center gap-4"><Trophy size={24} /><p className="text-[10px] font-black uppercase tracking-[0.6em] italic">IFL 26 LEAGUE MASTER • EST 2022</p></div></footer>
    </div>
  );
};
